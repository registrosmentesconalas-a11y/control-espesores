...
def crear_pdf_formal(df_final, resumen, tol_p):
    ...
    t_1 = Table(t_req_d, colWidths=[130, 120, 110, 110])
    ...
    t_2 = Table(t_riesgo_d, colWidths=[130, 120, 110, 110])
    ...
    t_3 = Table(t_rollos_d, colWidths=[90, 80, 55, 75, 90, 90])
    ...
    try:
        ...
        # Procesamiento en la memoria RAM
...
# Distribución del bloque de controles en columnas web
...
if archivo_cargado is not None:
    try:
        ...
        if not df_datos_cargados.empty:
            # --- 1. EVALUACIÓN ROLLO POR ROLLO ABSOLUTAMENTE INDEPENDIENTE ---
            est_l = []
            for _, fila in df_datos_cargados.iterrows():
                desv_abs = abs(fila['Desviación Real (in)'])
                
                # Clasificación analítica estricta por pieza individual sin promediar con el grupo
                if desv_abs <= 0.004:
                    est_l.append("RIESGO BAJO")
                elif desv_abs <= TOLERANCIA_INTERNA:  # Valores de desviación entre 0.0041" y 0.0080"
                    est_l.append("RIESGO MODERADO")
                else:  # Valores superiores al límite de la planta (0.0081" o más)
                    est_l.append("ALTO RIESGO")
            
            # Inyección directa y definitiva en el DataFrame de visualización
            ...
            # Se genera una copia explícita para evitar contaminación o reescritura de datos cruzados
            resumen = calcular_riesgo_grupos(df_datos_cargados.copy(), tol_proveedor)
