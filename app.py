...
def crear_pdf_formal(df_final, resumen, tol_p):
    ...
    t_1 = Table(t_req_d, colWidths=[150, 110, 110, 110])
    ...
    t_2 = Table(t_riesgo_d, colWidths=[150, 110, 110, 110])
    ...
    t_3 = Table(t_rollos_d, colWidths=[90, 80, 60, 80, 90, 100])
...
st.markdown(f"**Estndar Fijo Planta (Norma Interna):** `±{TOLERANCIA_INTERNA}\"`")
...
if archivo_cargado is not None:
    try:
        ...
        if not df_datos_cargados.empty:
            ...
            # 1. EVALUACIÓN ESTRICTA INDIVIDUAL (Se ejecuta y se guarda inmediatamente)
            for _, fila in df_datos_cargados.iterrows():
                desv_abs = abs(fila['Desviación Real (in)'])
                
                if desv_abs <= 0.004:
                    est_l.append("RIESGO BAJO")
                elif desv_abs <= TOLERANCIA_INTERNA:  # Entre 0.0041" y 0.0080"
                    est_l.append("RIESGO MODERADO")
                else:  # Mayor a 0.0080"
                    est_l.append("ALTO RIESGO")
            
            # Se fija el resultado analítico directo en el DataFrame
