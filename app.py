...
def calcular_riesgo_por_rollo(df_entrada, tol_proveedor):
    """Calcula la probabilidad de rechazo gaussiana de forma individual para cada rollo."""
    ...
    for _, fila in df_entrada.iterrows():
        id_rollo = fila['Rollo']
        mat = fila['Material']
        cal_str = fila['Calibre']
        med = float(fila['Espesor Real (in)'])
        nom = float(fila['Nominal Estándar'])
        
        # Integración estadística de colas de Gauss para este rollo específico
        ...
        resumen.append({
            ...
            "Clave": f"{mat} {cal_str}",
            "Espesor Real": med,
            "Riesgo Probabilístico": r_pct,
            "Dictamen Estadístico": dictam,
            "Nominal": nom
        })
    return resumen
...
def crear_pdf_formal(df_final, resumen, tol_p):
    ...
    # Tabla Sección 1: Muestreo Físico Analítico
    story.append(Paragraph("1. Desglose Analítico por Unidad (Muestreo Físico)", h2_st))
    t_rollos_d = [[Paragraph("Rollo", h_style), Paragraph("Especificación", h_style), Paragraph("Espesor (in)", h_style), Paragraph("Desviación Real", h_style), Paragraph("Estatus Planta", h_style)]]
    est_rollos = []
    for idx, f in df_final.reset_index(drop=True).iterrows():
        ...
        t_rollos_d.append([Paragraph(str(f['Rollo']), c_style), Paragraph(f['Material'] + " " + f['Calibre'], c_style), Paragraph(f"{f['Espesor Real (in)']:.4f}\"", c_style), Paragraph(f"{f['Desviación Real (in)']:+4f}\"", c_style), p_e])
        est_rollos.append(('BACKGROUND', (4, idx+1), (4, idx+1), bg))
    t_1 = Table(t_rollos_d, colWidths=[110, 110, 85, 85, 110])
    t_1.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,0), colors.HexColor('#708090')), ('ALIGN', (0,0), (-1,-1), 'CENTER'), ('VALIGN', (0,0), (-1,-1), 'MIDDLE'), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor('#A0A0A0')), ('PADDING', (0,0), (-1,-1), 5)] + est_rollos))
    story.append(t_1)
    
    # Tabla Sección 2: Matriz de Riesgo Individualizada
    story.append(Paragraph("2. Diagnóstico Probabilístico y Matriz de Riesgo por Rollo", h2_st))
    t_riesgo_d = [[Paragraph("ID Rollo", h_style), Paragraph("Clave Técnica", h_style), Paragraph("Media Espesor", h_style), Paragraph("Prob. Rechazo", h_style), Paragraph("Dictamen Estadístico", h_style)]]
    est_riesgo = []
    for idx, r in enumerate(resumen):
        p_d, bg = (Paragraph(r['Dictamen Estadístico'], s_bajo), colors.HexColor('#C6EFCE')) if "BAJO" in r['Dictamen Estadístico'] else ((Paragraph(r['Dictamen Estadístico'], s_mod), colors.HexColor('#FFF2CC')) if "MODERADO" in r['Dictamen Estadístico'] else (Paragraph(r['Dictamen Estadístico'], s_alto), colors.HexColor('#FFC7CE')))
        t_riesgo_d.append([Paragraph(str(r['Rollo']), c_bold), Paragraph(r['Clave'], c_style), Paragraph(f"{r['Espesor Real']:.4f}\"", c_style), Paragraph(f"{r['Riesgo Probabilístico']:.2f}%", c_style), p_d])
        est_riesgo.append(('BACKGROUND', (4, idx+1), (4, idx+1), bg))
    t_2 = Table(t_riesgo_d, colWidths=[110, 110, 85, 85, 110])
    ...
    story.append(Paragraph("3. Distribución Estadística de Calidad Individualizada (Curvas de Gauss)", h2_st))
    ...
    try:
        ...
        for idx, r in enumerate(resumen):
            m_desv = r['Espesor Real'] - r['Nominal']
            y_g = stats.norm.pdf(x_desv, loc=m_desv, scale=sigma_p)
            plt.plot(x_desv, y_g, label=f"{r['Rollo']} ({r['Riesgo Probabilístico']:.1f}%)", linewidth=1.5)
        ...
        plt.axvline(-TOLERANCIA_INTERNA, color='red', linestyle=':')
        plt.title("Curvas de Distribución Gaussiana Individual", fontsize=11, color='#2b579a', weight='bold')
        ...
        story.append(Paragraph(f"<i>No se pudo renderizar la gráfica técnica: {str(e)}</i>", c_style))
...
if archivo_cargado is not None:
    try:
        ...
        if not df_datos_cargados.empty:
            ...
            styler_individual = df_datos_cargados.style.format(formatos_ind).map(
                colorear_matriz_resumen, 
                subset=['Estatus Planta']
            )
            st.dataframe(styler_individual, use_container_width=True)
            
            # --- TABLA 2: MATRIZ DE RIESGO INDIVIDUALIZADA POR ROLLO ---
            # El motor matemático procesa cada fila de forma aislada basándose en las curvas de Gauss individuales
            resumen_rollos = calcular_riesgo_por_rollo(df_datos_cargados.copy(), tol_proveedor)
            st.subheader("📋 Matriz Estratégica de Riesgo Técnico por Rollo (Análisis de Gauss)")
            
            df_resumen = pd.DataFrame(resumen_rollos)
            formatos_res = {
                'Espesor Real': '{:.4f}"',
                'Riesgo Probabilístico': '{:.2f}%',
                'Nominal': '{:.3f}"'
            }
            styler_resumen = df_resumen.style.format(formatos_res).map(
                colorear_matriz_resumen, 
                subset=['Dictamen Estadístico']
            )
            st.dataframe(styler_resumen, use_container_width=True)
            
            # --- 3. GRÁFICA INTERACTIVA PLOTLY ---
            st.subheader("📈 Distribución Probabilística por Unidad (Campanas de Gauss)")
            fig = go.Figure()
            x_desv = np.linspace(-0.015, 0.015, 400)
            colores = ['#1f77b4', '#ff7f0e', '#2ca02c', '#9467bd', '#00ced1', '#ff1493', '#7cfc00']
            sig_t = tol_proveedor / 3.0
            
            for idx, r in enumerate(resumen_rollos):
                m_desv = r['Espesor Real'] - r['Nominal']
                y_g = stats.norm.pdf(x_desv, loc=m_desv, scale=sig_t)
                fig.add_trace(go.Scatter(
                    x=x_desv, y=y_g, mode='lines', 
                    name=f"Rollo {r['Rollo']} ({r['Riesgo Probabilístico']:.1f}%)", 
                    line=dict(color=colores[idx % len(colores)], width=2.5)
                ))
            ...
            pdf_buffer = crear_pdf_formal(df_datos_cargados, resumen_rollos, tol_proveedor)
            st.download_button(
